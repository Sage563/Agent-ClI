...
export class Config {
  ...
  private ensureDefaults() {
    ...
    if (typeof this.config.fast_mission !== "boolean") this.config.fast_mission = false;
  ...
  getAccessMode(): string {
    return this.get("access_scope", "limited");
  }

  showCost(): boolean {
    return this.get("command_log_enabled", true);
  }
  ...
  isFastMission() {
    return Boolean(this.config.fast_mission);
  }
  setFastMission(enabled: boolean) {
    this.config.fast_mission = enabled;
    this.save();
  }
}
