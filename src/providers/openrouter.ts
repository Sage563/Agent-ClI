...
export class OpenRouterProvider extends ResilientProvider {
    constructor() {
        super({ name: "openrouter", timeout: 60000, maxRetries: 3 });
    }
    ...
    protected async executeCall(system: string, task: TaskPayload, opts?: ProviderCallOptions): Promise<ProviderResult> {
        const apiKey = cfg.getApiKey("openrouter");
        if (!apiKey) throw new Error("OpenRouter API key not found. Run '/connect' or set OPENROUTER_API_KEY.");

        const providerConfig = cfg.getProviderConfig("openrouter");
        const thinkingEnabled = typeof providerConfig.thinking === "boolean" ? providerConfig.thinking : true;
        const baseURL = String(providerConfig.endpoint || OPENROUTER_BASE_URL);
        ...
        const model = String(providerConfig.model || "anthropic/claude-sonnet-4-20250514");
    ...
    protected async executeValidation() {
        const apiKey = cfg.getApiKey("openrouter");
        if (!apiKey) return { ok: false, message: "OpenRouter API key not set." };
        try {
            const providerConfig = cfg.getProviderConfig("openrouter");
            const baseURL = String(providerConfig.endpoint || OPENROUTER_BASE_URL);
            ...
            return { ok: false, message: `OpenRouter validation failed: ${String(error)}` };
        }
    }
}
