...
export class GroqProvider extends ResilientProvider {
    constructor() {
        super({ name: "groq", timeout: 30000, maxRetries: 3 });
    }
    ...
    protected async executeCall(system: string, task: TaskPayload, opts?: ProviderCallOptions): Promise<ProviderResult> {
        const apiKey = cfg.getApiKey("groq");
        if (!apiKey) throw new Error("Groq API key not found. Run '/connect' or set GROQ_API_KEY.");

        const providerConfig = cfg.getProviderConfig("groq");
        const baseURL = String(providerConfig.endpoint || GROQ_BASE_URL);
        ...
        const model = String(providerConfig.model || "llama-3.3-70b-versatile");
    ...
    protected async executeValidation() {
        const apiKey = cfg.getApiKey("groq");
        if (!apiKey) return { ok: false, message: "Groq API key not set." };
        try {
            const providerConfig = cfg.getProviderConfig("groq");
            const baseURL = String(providerConfig.endpoint || GROQ_BASE_URL);
            ...
            return { ok: false, message: `Groq validation failed: ${String(error)}` };
        }
    }
}
