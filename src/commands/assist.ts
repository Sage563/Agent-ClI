...
function usageText() {
  return [
    ...
    "",
    "Examples:",
    "- /assist explain src/core/agent.ts",
    "- /assist fix flaky session test",
    "- /assist tests src/tests/command_registry.test.ts",
    "- /assist review current branch changes",
  ].join("\n");
}
...
function promptFor(action: AssistAction, targetRaw: string) {
  ...
  if (action === "explain") {
    return `Explain ${target} clearly. Include architecture, key behavior, and risks.`;
  }
  if (action === "fix") {
    return `Diagnose and fix ${target}. Prefer minimal, safe edits and include a short validation checklist.`;
  }
  if (action === "tests") {
    return `Create or improve tests for ${target}. Focus on behavior coverage and edge cases.`;
  }
  if (action === "docs") {
    return `Write concise developer documentation for ${target} with quickstart and examples.`;
  }
  if (action === "review") {
    return `Review ${target} with a code-review mindset. Prioritize bugs, regressions, and missing tests.`;
  }
  return `Write a conventional commit message for ${target}. Include title and bullet-point body.`;
}
